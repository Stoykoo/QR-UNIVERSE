...
import { darkSwal } from "../utils/swalTheme";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        darkSwal.fire({ icon: "error", title: "Ups", text: msg });
      }, wait);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-full pl-10 pr-4 py-2.5 text-sm bg-slate-900/70 border border-slate-700 
                         focus:outline-none focus:ring-2 focus:ring-cyan-400 text-slate-100 
        ...
        <motion.button
          ...
          className="w-full mt-2 rounded-full bg-cyan-500 text-slate-950 font-semibold py-2.5 text-sm 
                     shadow-lg shadow-cyan-500/40 hover:bg-cyan-400 transition-transform 
                     disabled:opacity-60 disabled:cursor-not-allowed"
        >
          <span className="flex items-center justify-center gap-2">
            {loading && (
              <span className="w-4 h-4 border-2 border-slate-900/50 border-t-transparent 
